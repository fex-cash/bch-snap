 ...
 * (BCH) Virtual Machine Bytecode (VMB) tests (`bch_vmb_testson` and all
 * `bch_vmb_tests_*on` files). Compiling from this file allows us to easily
...
import { bigIntToBinUint64LE, binToHex, cashAssemblyToBin, hashTransactionUiOrder, hexToBin, range } from '../lib';

import { slot0Scenario, slot2Scenario, slot9Scenario } from './bch-vmb-test-mixins';
import { vmbTestGroupToVmbTests } from './bch-vmb-test-utils';
import { cashTokenTestDefinitionsBCH } from './bch-vmb-tests.cashtokens';
import { signingSerializationTestDefinitionsBCH } from './bch-vmb-tests.signing-serialization';

/**
 * The source data structure used to generate the Libauth BCH VMB test
 * vectors (`bch_vmb_testson` and all `bch_vmb_*_txon` files).
 */
