...
/**
 * Method extracted from Emscripten's preamble
 */
...
/**
 * Method derived from Emscripten's preamble
 */
 ...
 * using Node or DOM-specific APIs, you can use {@link instantiateSecp256k1}.
 *
 * Note, most of this method is translated and boiled-down from Emscripten's
 * preamble. Significant changes to the WASM build or breaking updates to
...
export const instantiateSecp256k1WasmBytes = async (
  ...
  if (!isLittleEndian(wasmMemory.buffer)) {
     ...
     * mock) and B) extracted from Emscripten's preamble, where it should
     * be tested properly.
     */
