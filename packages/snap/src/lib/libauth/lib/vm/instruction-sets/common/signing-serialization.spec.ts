...
import sighashTests from '../xec/fixtures/satoshi-client/sighashon' assert { type: 'json' };
...
pendingTests.map((expectation, currentTest) => {
  test.skip(`[signing-serialization tests] sighashon ${currentTest}/${pendingTests.length} (#${expectation.testIndex})`, (t) => {
